...
def evaluate_vip_expanded(num):
    ...
    # 3. مقطعين متطابقين تماماً: ABCDABCD (مثل تكرار النصفين تماماً)
    if d[:4] == d[4:]:
...
@app.route('/')
def home():
    return '<h2 style="font-family:Arial;text-align:center;padding:50px">🚀 Free Mobile VIP Bot (Strict Filter) يعمل بنجاح</h2>'


def run_smart_monitor():
    print('🔥🔥🔥 [THREAD ACTIVE] محرك الفحص الصارم بدأ', flush=True)
    current_proxies = []
...
if __name__ == '__main__':
    print('🚀 [START] Free Mobile VIP Bot (Strict Filter)', flush=True)
